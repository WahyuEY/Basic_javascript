...
function perkalianUnik(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < arr.length; j++) {
      if (i !== j) { // cek apakah index i dan j tidak sama
        product *= arr[j]; // kalikan angka pada index j ke product
      }
    }
    result.push(product); // masukkan product ke dalam array result
  }
  return result;
}
