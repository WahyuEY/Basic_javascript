...
function howManyGifts(maxBudget, gifts){
  // Code here
  gifts.sort((a, b) => a - b); // Urutkan array dari harga termurah ke termahal
  let numOfGifts = 0;
  for (let gift of gifts) {
    if (maxBudget >= gift) {
      // Jika masih cukup budget untuk membeli hadiah
      numOfGifts++;
      maxBudget -= gift; // Kurangi budget dengan harga hadiah yang dibeli
    } else {
      // Jika budget tidak mencukupi untuk membeli hadiah selanjutnya, keluar dari perulangan
      break;
    }
  }
  return numOfGifts;
